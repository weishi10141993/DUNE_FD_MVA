...
void CafChi2Test(void) {
   ...
   double scalefactor = scaleCorrection * POTperYear * 3.5; //3.5 years of data-taking
   ...
   TH1D* hDefaultSignal = new TH1D("TotalSignal", "TotalSignal", 25, 0., 10.);
   TH1D* hDefaultWLBkgdTwo = new TH1D("TotalWLBkgdTwo", "TotalWLBkgdTwo", 25, 0., 10.);
   TH1D* hDefaultWLBkgdOne = new TH1D("TotalWLBkgdOne", "TotalWLBkgdOne", 25, 0., 10.);
   TH1D* hDefaultWSBkgd = new TH1D("TotalWSBkgd", "TotalWSBkgd", 25, 0., 10.);
   TH1D* hDefaultIntBkgd = new TH1D("TotalIntBkgd", "TotalIntBkgd", 25, 0., 10.);
   TH1D* hDefaultNCBkgd = new TH1D("TotalNCBkgd", "TotalNCBkgd", 25, 0., 10.);
   TH1D hDefaultHist = TH1D("TotalHist", "TotalHist", 25, 0., 10.);
   ...
   for (int i = 0; i < 50; i++) {
      ...
      TH1D* dummySignal = new TH1D(TString::Format("Signal%d", i), TString::Format("Signal%d", i), 25, 0., 10.);
      signal_hists.push_back(*dummySignal);

      TH1D* dummyWLBkgdTwo = new TH1D(TString::Format("WLBkgdTwo%d", i), TString::Format("WLBkgdTwo%d", i), 25, 0., 10.);
      WLBkgdTwo_hists.push_back(*dummyWLBkgdTwo);

      TH1D* dummyWLBkgdOne = new TH1D(TString::Format("WLBkgdOne%d", i), TString::Format("WLBkgdOne%d", i), 25, 0., 10.);
      WLBkgdOne_hists.push_back(*dummyWLBkgdOne);

      TH1D* dummyWSBkgd = new TH1D(TString::Format("WSBkgd%d", i), TString::Format("WSBkgd%d", i), 25, 0., 10.);
      WSBkgd_hists.push_back(*dummyWSBkgd);

      TH1D* dummyIntBkgd = new TH1D(TString::Format("IntBkgd%d", i), TString::Format("IntBkgd%d", i), 25, 0., 10.);
      IntBkgd_hists.push_back(*dummyIntBkgd);

      TH1D* dummyNCBkgd = new TH1D(TString::Format("NCBkgd%d", i), TString::Format("NCBkgd%d", i), 25, 0., 10.);
      NCBkgd_hists.push_back(*dummyNCBkgd);

      TH1D* dummyTotal = new TH1D(TString::Format("Total%d", i), TString::Format("Total%d", i), 25, 0., 10.);
      Total_hists.push_back(*dummyTotal);

      TH1D* dummyBkgdHist = new TH1D(TString::Format("Bkgd%d", i), TString::Format("Bkgd%d", i), 25, 0., 10.);
   ...
   Right now, this macro is configured to create a stacked hist for nu_mu selection.
   To change this, change:
      the CVN cut in line 298
      the filename cut in line 305
      the PDG cut in line 304,344,353
   ...
   */
   ...
   for (int bin = 0; bin <= hDefaultHist.GetNcells(); bin++) {
     ...
     //std::cout << hDefaultHist.GetBinContent(bin) << std::endl;
   ...
   hDefaultSignal->SetFillColor(kBlue);
   ...
   hDefaultWLBkgdOne->SetFillColor(kRed);
   ...
   if (!useNuMu) {
      leg->AddEntry(hDefaultSignal, "#nu_{#mu} Signal");
      leg->AddEntry(hDefaultWLBkgdOne, "#nu_{#tau} Background");
      leg->AddEntry(hDefaultWSBkgd, "#bar{#nu_{#mu}} Background");
   }
   else {
      leg->AddEntry(hDefaultSignal, "#nu_{#tau} Signal");
      leg->AddEntry(hDefaultWLBkgdOne, "#nu_{#mu} Background");
      leg->AddEntry(hDefaultWSBkgd, "#bar{#nu_{#tau}} Background");
   }

   leg->AddEntry(hDefaultNCBkgd, "NC Background");
   leg->AddEntry(hDefaultWLBkgdTwo, "#nu_{e} Background");
   //leg->AddEntry(hDefaultIntBkgd, "Intrinsic Background");   -- This bit is usually too small to see

   hDefaultStack->Draw("HIST");

   if (!useNuMu) {
      if (!useRHC) {
         hDefaultStack->SetTitle("#nu_{#tau} Selection with CAFAna (FHC -- 3.5 POT Years)");
      }
      else {
         hDefaultStack->SetTitle("#nu_{#tau} Selection with CAFAna (RHC -- 3.5 POT Years)");
      }
   }
   else {
      if (!useRHC) {
         hDefaultStack->SetTitle("#nu_{#mu} Selection with CAFAna (FHC -- 3.5 POT Years)");
      }
      else {
         hDefaultStack->SetTitle("#nu_{#mu} Selection with CAFAna (RHC -- 3.5 POT Years)");
      }
   }
   ...
   if (!useEHadBinning) {
      ...
      hDefaultStack->GetXaxis()->SetTitle("Reco. E_{Had} (GeV)");
   }

   hDefaultStack->GetYaxis()->SetTitle("Counts");

   //I don't have the stats for the RHC caf cut. These can be easily obtained from CafCutEval.C
   TLatex *tb1, *tb2, *tb3 = NULL;

   if (!useNuMu) {
      tb1 = new TLatex(5.8, 750, "#scale[0.7]{Efficiency: 0.7103}");
      tb2 = new TLatex(5.8, 650, "#scale[0.7]{Purity: 0.02007}");
      tb3 = new TLatex(5.8, 550, "#scale[0.7]{Total S / #sqrt{B}: 2.23167}");
   }
   else {
      tb1 = new TLatex(5.8, 750, "#scale[0.7]{Efficiency: 0.788644}");
      tb2 = new TLatex(5.8, 650, "#scale[0.7]{Purity: 0.895904}");
      tb3 = new TLatex(5.8, 550, "#scale[0.7]{Total S / #sqrt{B}: 380.511}");
   }
   ...
   if (!useNuMu) {
      if (!useRHC) {
         if (!useEHadBinning) {
            c1->SaveAs("./Figures/cafFHCTauStackedHistTrue.png");
         }
         else {
            c1->SaveAs("./Figures/cafFHCTauStackedHistReco.png");
         }
      }
      else {
         if (!useEHadBinning) {
            c1->SaveAs("./Figures/cafRHCTauStackedHistTrue.png");
         }
         else {
            c1->SaveAs("./Figures/cafRHCTauStackedHistReco.png");
         }
      }
   }
   else {
      if (!useRHC) {
         if (!useEHadBinning) {
            c1->SaveAs("./Figures/cafFHCMuStackedHistTrue.png");
         }
         else {
            c1->SaveAs("./Figures/cafFHCMuStackedHistReco.png");
         }
      }
      else {
         if (!useEHadBinning) {
            c1->SaveAs("./Figures/cafRHCMuStackedHistTrue.png");
         }
         else {
            c1->SaveAs("./Figures/cafRHCMuStackedHistReco.png");
         }
      }
   }
   ...
   //Comment out the SaveAs commands for nu_mu testing (or when you want to avoid overwriting images)

   if (!useRHC) {
      if (!useEHadBinning) {
         theta_23Chi2Graph->SetTitle("FHC dchi2 #theta_{23} (cafAna cut -- True E binning)");
      }
      else {
         theta_23Chi2Graph->SetTitle("FHC dchi2 #theta_{23} (cafAna cut -- EHad binning)");
      }
   }
   else {
      if (!useEHadBinning) {
         theta_23Chi2Graph->SetTitle("RHC dchi2 #theta_{23} (cafAna cut -- True binning)");
      }
      else {
         theta_23Chi2Graph->SetTitle("RHC dchi2 #theta_{23} (cafAna cut -- EHad binning)");
      }
   }

   theta_23Chi2Graph->GetXaxis()->SetTitle("sin^{2}(#theta_{23})");
   theta_23Chi2Graph->GetYaxis()->SetTitle("#chi^{2}");
   theta_23Chi2Graph->SetMarkerStyle(20);
   theta_23Chi2Graph->Draw("AP");

   /*if (!useRHC) {
      if (!useEHadBinning) {
         c1->SaveAs("./Figures/Chi2/cafFHCTheta23Chi2True.png");
      }
      else {
         c1->SaveAs("./Figures/Chi2/cafFHCTheta23Chi2Reco.png");
      }
   }
   else {
      if (!useEHadBinning) {
         c1->SaveAs("./Figures/Chi2/cafRHCTheta23Chi2True.png");
      }
      else {
         c1->SaveAs("./Figures/Chi2/cafRHCTheta23Chi2Reco.png");
      }
   }*/
   ...
   if (!useRHC) {
      if (!useEHadBinning) {
         delmsqChi2Graph->SetTitle("FHC dchi2 #Delta m^{2} (cafAna cut -- True E binning)");
      }
      else {
         delmsqChi2Graph->SetTitle("FHC dchi2 #Delta m^{2} (cafAna cut -- EHad binning)");
      }
   }
   else {
      if (!useEHadBinning) {
         delmsqChi2Graph->SetTitle("RHC dchi2 #Delta m^{2} (cafAna cut -- True binning)");
      }
      else {
         delmsqChi2Graph->SetTitle("RHC dchi2 #Delta m^{2} (cafAna cut -- EHad binning)");
      }
   }
   ...
   delmsqChi2Graph->SetMarkerStyle(20);
   delmsqChi2Graph->Draw("AP");

   /*if (!useRHC) {
      if (!useEHadBinning) {
         c1->SaveAs("./Figures/Chi2/cafFHCDeltaM2Chi2True.png");
      }
      else {
         c1->SaveAs("./Figures/Chi2/cafFHCDeltaM2Chi2Reco.png");
      }
   }
   else {
      if (!useEHadBinning) {
         c1->SaveAs("./Figures/Chi2/cafRHCDeltaM2Chi2True.png");
      }
      else {
         c1->SaveAs("./Figures/Chi2/cafRHCDeltaM2Chi2Reco.png");
      }
   }*/
   ...
   /*theta_23Zscore->SetTitle("FHC Z-Score (cafAna Cut -- 3.5 POT Years)");
   ...
   c1->SaveAs("./Figures/Chi2/FHCcafAnaTheta23ZScore.png");
   c1->Clear();
   c1->Update();

   delmsqZscore->SetTitle("FHC Z-Score (cafAna Cut -- 3.5 POT Years)");
   ...
   c1->SaveAs("./Figures/Chi2/FHCcafAnaDeltaM2ZScore.png");
   c1->Clear();
   c1->Update();*/
   ...
   /*for (int i = 0; i < 25; i++) {
      ...
      legends[i].Draw();
      c1->SaveAs(TString::Format("./Figures/Chi2/dummyhists/cafAnaStackedHist%d.png", i));
      c1->Clear();
      c1->Update();
